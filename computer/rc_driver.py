...
class SensorDataHandler(socketserver.BaseRequestHandler):
    ...
    def handle(self):
        global sensor_data
        while self.data:
            sensor_data = self.request.recv(1024)
            sensor_data = struct.unpack('!d', sensor_data)
            sensor_data = round(float(sensor_data[0]), 1)
...
class VideoStreamHandler(socketserver.StreamRequestHandler):
    ...
    stop_start = 5  # start time when stop at the stop sign
    stop_finish = 0
    stop_time = 5
        ...
        try:
            ...
            while True:
                ...
                if first != -1 and last != -1:
                    ...
                    #cv2.imshow('image_array', roi.reshape(1, int(height/2) * width))
                    # cv2.imshow('mlp_image', image_array)
